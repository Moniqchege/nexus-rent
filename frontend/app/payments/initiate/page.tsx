      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        ...
        <GlassPanel style={{ flex: 1, minWidth: 300 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 20 }}>
            {methods.map((m) => (
              <div key={m.key} onClick={() => { setMethod(m.key); setStkSent(false); setBankRef(""); }}
          ...
          {method === "bank" && (
            <div>
              <NeonButton variant="primary" onClick={() => setBankRef(`NEXUS-BANK-${Math.floor(Date.now() / 1000).toString().slice(-6)}`)}>
              ...
              {bankRef && (
                <div style={{ marginTop: 14, padding: "14px 16px", borderRadius: 12, background: "rgba(96,165,250,0.06)", border: "1px solid rgba(96,165,250,0.2)" }}>
                  ...
                  <div style={{ marginTop: 8, fontSize: 11, color: "rgba(255,255,255,0.3)" }}>Verify via <code style={{ color: "#a78bfa", fontFamily: "monospace" }}>PUT /api/payments/:id/verify</code></div>
                </div>
              )}
            </div>
          )}
