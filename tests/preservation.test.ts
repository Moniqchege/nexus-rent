...
test("Preservation 5: Reports page includes propertyId and month params in CSV export", async () => {
  ...
  assert.ok(fileContent.includes("URLSearchParams({ month })"), "Export CSV includes month param");
  assert.ok(fileContent.includes('params.set("propertyId", propertyId)'), "Export CSV includes propertyId param when set");
});
