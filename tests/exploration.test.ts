...
test("Bug 5b — KRA download unauthenticated", async () => {
  ...
  const usesAuthenticatedApiForExport = feContent.includes('api.get("/api/payments/reports"');

  console.log("Bug 5b Counterexample:", { usesAuthenticatedApiForExport });

  assert.strictEqual(usesAuthenticatedApiForExport, true, "BUG CONFIRMED: KRA Report uses unauthenticated window.open");
});
